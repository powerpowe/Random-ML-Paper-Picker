...
def search_papers():
    ...
    papers = load_papers(
        [conf for conf, var in conf_vars.items() if var.get()],
        [year for year, var in year_vars.items() if var.get()]
    )
    filtered_papers = [p for p in papers if query in p['title'].lower()]
    
    listbox.delete(0, tk.END)
    for paper in filtered_papers:
        listbox.insert(tk.END, f"[{paper['conference']} {paper['year']}] {paper['title']}")
    listbox.papers = filtered_papers

ttk.Button(frame_search, text="Search", command=search_papers).pack(side=tk.LEFT, padx=5)
